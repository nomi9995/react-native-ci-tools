const mainAppDefaultPathPattern = 'app/src/main/java/com/developer/arsltech/github/SplashActivity.java';
...
const process = (androidProjectValidPath, payload, strategy) => {
	...
	return strategy.utils
		.globSearch(strategy.globber, mainAppPathPattern, false)
		.then((fileSearchResults) => {
			if (fileSearchResults) {
				if (fileSearchResults.length == 1) {
					return fileSearchResults[0];
				}
				throw new Error(`Unable to determine SplashActivity.java file, found (${fileSearchResults.length}) matches`);
			}
			throw new Error(
				`Unable to find SplashActivity.java file, search directory (${fileSearchResults.length}) matches`
			);
		})
